...
type RegistrationGRPCHandlers interface {
	StartOperation(ctx context.Context, operation_type uint8, operation_data map[string]interface{}) error
	Process(ctx context.Context, saga_uuid uuid.UUID, saga *models.Saga, event_uuid uuid.UUID, event *models.Event, data map[string]interface{}) error
